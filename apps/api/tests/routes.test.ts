...
describe("Streams, Milestones and Stats Routes", () => {
	...
	beforeAll(async () => {
		...
		stacksServiceMock = {
			...
			getMilestoneStream: vi.fn().mockImplementation(async (id) => mockMilestoneStreams[id] || null),
		};
		...
		streamIndexerMock = {
			...
			getSenderStreams: vi.fn().mockImplementation((sender) => mockStreams.filter((s) => s.sender === sender)),
			getRecipientStreams: vi.fn().mockImplementation((recipient) => mockStreams.filter((s) => s.recipient === recipient)),
			getStreamView: vi.fn().mockImplementation(async (id) => {
				const stream = mockStreams.find((s) => s.id === id);
				return stream ? { ...stream, depositAmount: stream.depositAmount.toString(), claimedAmount: stream.claimedAmount.toString() } : null;
			}),
			...
			getStreamHistory: vi.fn().mockImplementation((id) => [
				{ eventType: "stream-created", streamId: id, blockHeight: 100, txId: "tx-1", eventIndex: 0, caller: "ST1" },
			]),
	...
	describe("GET /api/streams", () => {
		...
		it("filters streams by sender address", async () => {
			const res = await fetch(`http://127.0.0.1:${apiPort}/api/streams?sender=ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM`);
			expect(res.status).toBe(200);
			const body = await res.json();
