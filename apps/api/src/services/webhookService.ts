	...
	private async deliverWithRetry(sub: WebhookSubscription, event: StreamEvent): Promise<void> {
		...
		while (!success && attempt <= retries.length) {
			...
			try {
				...
				if (response.ok) {
					...
					console.warn(`Webhook subscription ${sub.id} returned status ${response.status} on attempt ${attempt + 1}`);
				}
			} catch (error: any) {
				console.warn(`Webhook subscription ${sub.id} failed attempt ${attempt + 1}: ${error.message}`);
			}
		...
		if (success) {
			...
			if (sub.consecutiveFailures >= 10) {
				sub.isActive = false;
				console.warn(`Webhook subscription ${sub.id} deactivated due to 10 consecutive delivery failures.`);
			}
			await this.save();
		}
	}
