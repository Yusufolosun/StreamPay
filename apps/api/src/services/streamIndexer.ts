...
import {
  ...
  type StreamBalanceInput,
  type StreamBalanceSnapshot,
...
export class StreamIndexer {
  ...
  private async fetchEventsSince(cursor: number, tip: number): Promise<StreamEvent[]> {
    ...
    while (fetchMore) {
      ...
      while (attempt <= maxRetries) {
        try {
          events = await this.stacksService.getContractEvents(this.contractAddress, {
            limit,
            offset,
          });
          break;
          ...
          if (attempt > maxRetries) {
            console.error(
              `Failed to fetch events at offset ${offset} after ${maxRetries} retries:`,
              error,
            );
            throw error;
          }
          ...
          console.warn(
            `Retrying event fetch (attempt ${attempt}/${maxRetries}) after ${delay}ms...`,
          );
  ...
  private async dispatchEvent(event: StreamEvent): Promise<void> {
    if (event.streamId != null) {
      ...
      const exists = history.some(
        (h) => h.txId === event.txId && h.eventIndex === event.eventIndex,
      );
      if (!exists) {
        history.push(event);
        history.sort((a, b) => {
  ...
  private async handleStreamCreated(
    event: StreamEvent & { eventType: 'stream-created' },
  ): Promise<void> {
