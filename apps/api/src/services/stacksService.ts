...
export function mapEventTupleToStreamEvent(
  ...
  const blockHeightVal =
    parsed['block-height'] !== undefined ? parsed['block-height'] : txBlockHeight;
  const blockHeight =
    typeof blockHeightVal === 'bigint' ? Number(blockHeightVal) : Number(blockHeightVal || 0);
...
export class StacksService {
  ...
  public async getStreamById(streamId: number): Promise<OnChainStream | null> {
    ...
    const stream = await withRetry(async () => {
      ...
      const resultHex = await this.callReadOnly(contractAddress, contractName, 'get-stream', [
