...
const resolveEffectiveBlock = (input: StreamBalanceInput): number => {
  ...
  if (typeof input.pausedAtBlock === 'number') {
    effectiveBlock = Math.min(
      effectiveBlock,
      clampBlockHeight(input.pausedAtBlock, input.startBlock),
    );
  }
  ...
  if (typeof input.cancelledAtBlock === 'number') {
    effectiveBlock = Math.min(
      effectiveBlock,
      clampBlockHeight(input.cancelledAtBlock, input.startBlock),
    );
  }
...
export const calculateStreamBalance = (input: StreamBalanceInput): StreamBalanceSnapshot => {
  ...
  const cappedAmount =
    input.fundedAmount == null ? streamedAmount : minBigInt(streamedAmount, input.fundedAmount);
  ...
  const remainingAmount =
    input.fundedAmount == null
      ? 0n
      : input.fundedAmount > cappedAmount
        ? input.fundedAmount - cappedAmount
        : 0n;
...
export const calculateClaimableBalance = (
  stream: StreamIndexEntry,
  currentBlock: number,
): bigint => {
  ...
  const lastCheckpointBlock =
    stream.startBlock + Number(stream.claimedAmount / stream.ratePerBlock);
...
export const calculateMilestoneAmounts = (
  milestoneStream: OnChainMilestoneStream,
): MilestoneAmounts[] => {
...
export const calculateStreamProgress = (
  stream: StreamIndexEntry,
  currentBlock: number,
): StreamProgress => {
  ...
  const estimatedEndDate = new Date(Date.now() + blocksRemaining * 5 * 1000);
