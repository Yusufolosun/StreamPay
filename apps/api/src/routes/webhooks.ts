...
export const createWebhooksRouter = (webhookService?: WebhookService): Router => {
		...
		asyncHandler(async (request, response) => {
			...
			for (const event of events) {
				if (!VALID_EVENTS.includes(event)) {
					throw createApiError(400, "invalid_event_type", `Event type '${event}' is not supported.`);
				}
			}
	...
	router.get(
		...
		asyncHandler(async (request, response) => {
			...
			if (!sub) {
				throw createApiError(404, "subscription_not_found", `Subscription with ID ${request.params.id} was not found.`);
			}
