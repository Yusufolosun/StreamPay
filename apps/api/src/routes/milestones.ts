...
export const createMilestonesRouter = (stacksService?: StacksService): Router => {
	...
	router.get(
		...
		asyncHandler(async (request, response) => {
			...
			if (typeof sender === "string" && !isValidStacksAddress(sender)) {
				throw createApiError(400, "invalid_sender", "The provided sender address is not a valid Stacks address.");
			}
			if (typeof recipient === "string" && !isValidStacksAddress(recipient)) {
				throw createApiError(400, "invalid_recipient", "The provided recipient address is not a valid Stacks address.");
			}
			if (typeof arbiter === "string" && !isValidStacksAddress(arbiter)) {
				throw createApiError(400, "invalid_arbiter", "The provided arbiter address is not a valid Stacks address.");
			}
			if (typeof participant === "string" && !isValidStacksAddress(participant)) {
				throw createApiError(400, "invalid_participant", "The provided participant address is not a valid Stacks address.");
			}
			...
			for (let id = 1; id <= nonce; id++) {
				fetchPromises.push(
					stacksService.getMilestoneStream(id).then((stream) => (stream ? { id, ...stream } : null)),
				);
			}
			...
			if (typeof participant === "string") {
				filteredStreams = filteredStreams.filter(
					(s) => s.sender === participant || s.recipient === participant || s.arbiter === participant,
				);
			}
	...
	router.get(
		...
		asyncHandler(async (request, response) => {
			...
			if (!stream) {
				throw createApiError(404, "milestone_stream_not_found", `Milestone stream with ID ${milestoneStreamId} was not found.`);
			}
	...
	router.get(
		...
		asyncHandler(async (request, response) => {
			...
			if (!stream) {
				throw createApiError(404, "milestone_stream_not_found", `Milestone stream with ID ${id} was not found.`);
			}
