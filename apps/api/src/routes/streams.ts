...
import { calculateClaimableBalance, calculateStreamProgress } from "../services/balanceCalculator.js";
...
export const createStreamsRouter = (streamIndexer?: StreamIndexer): Router => {
	...
	router.get(
		...
		asyncHandler(async (request, response) => {
			...
			if (typeof address === "string" && !isValidStacksAddress(address)) {
				throw createApiError(400, "invalid_address", "The provided address is not a valid Stacks address.");
			}
			...
			if (typeof status === "string") {
				...
				if (!validStatuses.includes(status)) {
					throw createApiError(400, "invalid_status", `Status must be one of: ${validStatuses.join(", ")}`);
				}
	...
	router.get(
		...
		asyncHandler(async (request, response) => {
			...
			const percentClaimed = totalDeposited > 0n
				? Number((totalClaimed * 10000n) / totalDeposited) / 100
				: 0;
